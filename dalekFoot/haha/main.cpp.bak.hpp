...
int main_ros() {
    ...
    mcap::Status status =
      writer.open("runtime/output.mcap", mcap::McapWriterOptions("testNdoe"));
    if (!status.ok()) {
        // ERROR
        return -1;
    }
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        // serialization library. In this example, we manually create ROS1
        // binary data
...
int main() {
    mcap::McapWriter writer;
    auto s =
      writer.open("runtime/output.mcap", mcap::McapWriterOptions("testnode"));
    if (!s.ok()) {
        return -1;
    }
    ...
    writer.addSchema(
      path_schema);    // Assigned schema id is written to path_schema.id
    // Register Channel
    mcap::Channel path_channel("/planner/path", "protobuf", path_schema.id);
    writer.addChannel(
      path_channel);    // Assigned channel id written to path_channel.id
