...
std::uint32_t calculateCrc(const cooboc::comm::GHPacket &spiPacket) {
...
int main() {
    ...
    cooboc::comm::GHPacket rxPacket {};
    cooboc::comm::HGPacket txPacket {};
    ...
    while (true) {
        ...
        for (std::size_t offset {0U}; offset < 10U; offset++) {
            ...
            for (std::size_t i {0U}; i < 4U; ++i) { txPacket.wheelsPlanning[i][offset] = value; }
        }
        // std::cout << "o" << std::flush;
        std::uint32_t expectedCrc = calculateCrc(txPacket);
        txPacket.crc              = expectedCrc;

        // Validate last receive
        {
            std::uint32_t expectRxCrc = calculateCrc(rxPacket);
            // std::printf("expect: %u, actual: %u\r\n", expectRxCrc, rxPacket.crc);
            if (rxPacket.crc == expectRxCrc) {
                std::printf("odo: %d %d %d %d\r\n",
                            rxPacket.odometry[0],
                            rxPacket.odometry[1],
                            rxPacket.odometry[2],
                            rxPacket.odometry[3]);
        ...
        while (true) {
            ...
            if (duration > 10000) {
                ...
                rxPacket = ch341.sendPacket(txPacket);
                break;
            }
