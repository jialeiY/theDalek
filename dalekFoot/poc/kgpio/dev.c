...
{
    printk(KERN_NOTICE "Sample-driver: Device file is read at offset = %i, read bytes count = %u", (int)*position, (unsigned int)count);
...
static int register_device(void)
{
    ...
    printk(KERN_NOTICE "Sample-driver: registered character device with major number = %i and minor numbers 0...255", device_file_major_number);

    // Request GPIO

    err = gpio_request(16, "Cooboc");
    if (err != 0)
    {
    ...
    // Set output
    err = gpio_direction_output(16, 1);
    if (err != 0)
    {
